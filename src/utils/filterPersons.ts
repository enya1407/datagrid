...
const stringFilter = (filterBy: keyof PersonType, searchedValue: string) => (
  ...
  (el[filterBy] as string).toLowerCase().includes(searchedValue.toLowerCase());
...
export const filterPersons = (
  ...
  if (
    (!filterBy.first_name && !filterBy.last_name && !filterBy.app_name) ||
    !searchedValue
  )
...
export const filterPersonsByGender = (
  dataPersons: Array<PersonType>,
...
export const filterPersonsByShirtSize = (
  dataPersons: Array<PersonType>,
  visibilityShirtSize: Record<keyof ShirtSize, boolean>
) => {
  const visibilityValue = Object.entries(visibilityShirtSize)
    .filter((el) => el[1] === true)
    .map((el) => el[0]);
  if (
    ...
    return dataPersons.filter((el) =>
      visibilityValue.includes(el['shirt_size'])
    );
  }
};
