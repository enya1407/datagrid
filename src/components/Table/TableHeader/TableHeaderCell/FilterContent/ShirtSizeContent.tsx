...
const ShirtSizeContent = () => {
  ...
  const arrShirtSize: Array<keyof ShirtSize> = [
    '3XL',
    '2XL',
    'XL',
    'L',
  ...
  const contentByShirtSize = arrShirtSize.map(
    (data: Partial<keyof ShirtSize>, i: number) => (
      <div key={i}>
        <input
          type="checkbox"
  ...
  return <div>{contentByShirtSize}</div>;
};
