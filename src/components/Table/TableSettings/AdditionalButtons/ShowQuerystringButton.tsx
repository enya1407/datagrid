...
const ShowQuerystringButton = () => {
  ...
  const writeDataToTheQuerystring = () => {
    const gender = Object.entries(visibilityGender)
      .filter((el) => el[1] === true)
      .map((el) => el[0])
      .join('+');
    const shirtSize = Object.entries(visibilityShirtSize)
      .filter((el) => el[1] === true)
      .map((el) => el[0])
      .join('+');
    const boolean = Object.entries(visibilityBoolean)
      .filter((el) => el[1] === true)
      .map((el) => el[0])
      .join('+');
  ...
  return (
    <Popover content={content} trigger="click">
      <Button
        title="copy querystring with current filters"
